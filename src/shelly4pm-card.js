...
class Shelly4PMCard extends HTMLElement {
  ...
  setConfig(config) {
    ...
    this.container.appendChild(shellyLogo);
    ...
    this.vacEl.textContent = "VAC: 0 V";

    this.totalPower = document.createElement("div");
    this.totalPower.textContent = "Total: 0 W";
  ...
  set hass(hass) {
    ...
    if (this.vacEntity && this.vacEl) {
      ...
      if (state && state.state !== "unknown" && state.state !== "unavailable") {
        const parsed = parseFloat(state.state);
        if (!isNaN(parsed)) value = parsed;
        unit = state.attributes.unit_of_measurement || "V";
      }
    ...
    const maxPower = 9600;
    let percent = Math.min((total / maxPower) * 100, 100);
  ...
  static getConfigElement() {
    return document.createElement("shelly4pm-card-editor");
  }
...
window.customCards.push({
  type: "shelly4pm-card",
  name: "Shelly 4PM Card",
  description: "Visual Shelly 4PM control card",
  preview: true
});
